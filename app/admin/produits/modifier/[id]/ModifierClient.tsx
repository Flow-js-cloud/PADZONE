...
import { getAllProducts, updateCustomProduct, overrideStaticProduct } from "@/lib/customProducts";
...
const GRADIENTS = [
  { label: "Cyan Gaming",  value: "linear-gradient(135deg, #020209 0%, #001a2e 50%, #003a5c 100%)" },
  { label: "Magenta",      value: "linear-gradient(135deg, #0a0010 0%, #2a0030 50%, #4a0020 100%)" },
  { label: "Neon Green",   value: "linear-gradient(135deg, #001510 0%, #003322 50%, #006633 100%)" },
  { label: "Dragon Ball",  value: "linear-gradient(135deg, #ff6600 0%, #ffd700 50%, #cc4400 100%)" },
  { label: "Naruto",       value: "linear-gradient(135deg, #ff4500 0%, #ff8c00 50%, #ffd700 100%)" },
  { label: "One Piece",    value: "linear-gradient(135deg, #fff8e1 0%, #ffd700 50%, #cc0000 100%)" },
  { label: "JJK",          value: "linear-gradient(135deg, #000066 0%, #0000cc 50%, #6666ff 100%)" },
  { label: "Valorant",     value: "linear-gradient(135deg, #1a0000 0%, #cc0000 50%, #ff4040 100%)" },
  { label: "LoL Gold",     value: "linear-gradient(135deg, #0a1628 0%, #1e3a5f 50%, #c89b3c 100%)" },
  { label: "CS2 Desert",   value: "linear-gradient(135deg, #c8a870 0%, #806030 50%, #604820 100%)" },
  { label: "RGB Spectrum", value: "linear-gradient(135deg, #ff0066, #00e5ff, #00ff99, #bf00ff)"    },
  { label: "Or",           value: "linear-gradient(135deg, #0f0a00 0%, #2a1a00 50%, #4a3000 100%)" },
  { label: "Marbre Blanc", value: "linear-gradient(135deg, #e8e8e8 0%, #f5f5f5 50%, #d0d0d0 100%)" },
  { label: "Cuir Marron",  value: "linear-gradient(135deg, #3b1f0a 0%, #6b3a1f 50%, #4a2810 100%)" },
  { label: "Nature Vert",  value: "linear-gradient(135deg, #1a2e1a 0%, #2d4a20 50%, #3d6030 100%)" },
  { label: "Pastel",       value: "linear-gradient(135deg, #ffd6e0 0%, #c9b8f5 50%, #b8e0ff 100%)" },
  { label: "Noir Uni",     value: "linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 50%, #0f0f0f 100%)" },
];

type FormData = {
  shortName: string; name: string; price: string; originalPrice: string;
  category: string; subCategory: string; size: string; dimensions: string;
  gradient: string; accentColor: string; description: string;
  features: string[]; stock: string;
  isNew: boolean; isBestSeller: boolean; isRGB: boolean; badge: string;
  images: string[];
};
...
export function ModifierClient() {
  ...
  if (!form) {
    return (
      <AdminShell title="Modifier produit">
        <p className="text-sm" style={{ color: "#475569" }}>Produit introuvable.</p>
      </AdminShell>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    if (isCustom) {
      ...
      overrideStaticProduct(id, updates);
    }
    setSaved(true);
  ...
  return (
    <AdminShell title="Modifier le produit">
      <div className="max-w-3xl">
        ...
        {!isCustom && (
          <div className="mb-5 px-4 py-3 rounded-xl text-xs" style={{ background: "#f59e0b15", border: "1px solid #f59e0b30", color: "#f59e0b" }}>
            Produit catalogue — les modifications créeront une copie personnalisée.
          </div>
        )}
        ...
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Nom court" required>
                <input value={form.shortName} onChange={e => set("shortName", e.target.value)}
                  ...
                  style={{ ...IS, borderColor: errors.shortName ? "#ef4444" : "#1e2a3a" }} />
                {errors.shortName && <p className="text-xs mt-1" style={{ color: "#ef4444" }}>{errors.shortName}</p>}
              </Field>
              <Field label="Nom complet" required>
                <input value={form.name} onChange={e => set("name", e.target.value)}
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none"
                  style={{ ...IS, borderColor: errors.name ? "#ef4444" : "#1e2a3a" }} />
              ...
              <Field label="Prix (€)" required>
                <input value={form.price} onChange={e => set("price", e.target.value)}
                  ...
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none"
                  style={{ ...IS, borderColor: errors.price ? "#ef4444" : "#1e2a3a" }} />
              ...
              <Field label="Prix barré (€)">
                <input value={form.originalPrice} onChange={e => set("originalPrice", e.target.value)}
                  type="number" step="0.01" placeholder="optionnel"
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none"
                  style={IS} />
              </Field>
              <Field label="Badge">
                <input value={form.badge} onChange={e => set("badge", e.target.value)}
                  placeholder="NOUVEAU, PROMO… (optionnel)"
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none"
                  style={IS} />
              </Field>
              <Field label="Stock" required>
                <input value={form.stock} onChange={e => set("stock", e.target.value)}
                  type="number"
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none"
                  style={{ ...IS, borderColor: errors.stock ? "#ef4444" : "#1e2a3a" }} />
              </Field>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <Field label="Catégorie" required>
                <select value={form.category}
                  ...
                  className="w-full px-3 py-2.5 rounded-lg text-sm outline-none" style={IS}>
          ...
          <div className="p-6 rounded-xl space-y-4" style={{ background: "#141921", border: "1px solid #1e2a3a" }}>
            ...
            <div>
              <p className="text-xs mb-2" style={{ color: "#475569" }}>Aperçu</p>
            ...
            <Field label="Couleur d'accentuation">
              <div className="flex items-center gap-3">
                ...
                <input value={form.accentColor} onChange={e => set("accentColor", e.target.value)}
                  className="w-32 px-3 py-2 rounded-lg text-sm outline-none font-mono" style={IS} />
          ...
          <div className="p-6 rounded-xl space-y-4" style={{ background: "#141921", border: "1px solid #1e2a3a" }}>
            ...
            <Field label="Description" required>
              <textarea value={form.description} onChange={e => set("description", e.target.value)}
                ...
                style={{ ...IS, borderColor: errors.description ? "#ef4444" : "#1e2a3a" }} />
          ...
          <div className="flex gap-3 pb-8">
            <button onClick={handleSubmit}
              ...
              {saved ? <><Check size={14} /> Sauvegardé !</> : <><Check size={14} /> Enregistrer les modifications</>}
            </button>
            <Link href="/admin/produits"
