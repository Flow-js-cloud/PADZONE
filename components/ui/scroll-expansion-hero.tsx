  ...
  return (
    <>
      ...
      <div className='lg:hidden overflow-x-hidden'>
        <section className='relative flex flex-col items-center justify-center overflow-hidden' style={{ height: '88svh' }}>
          ...
          <div className='relative z-10 text-center px-6 flex flex-col items-center gap-5 w-full max-w-sm'>
            <span className='animate-fade-up delay-150 font-mono text-[10px] tracking-[0.3em] uppercase px-4 py-1.5 rounded-full'
              style={{ color: "#00e5ff", border: "1px solid rgba(0,229,255,0.3)", background: "rgba(0,229,255,0.06)" }}>
              Tapis Gaming Premium
            </span>

            <h1 className='animate-fade-up delay-250 font-orbitron font-black text-5xl leading-[1.1] tracking-tight'>
              <span style={{ color: "#00e5ff", textShadow: "0 0 25px rgba(0,229,255,0.6)" }}>ÉLÈVE</span>
              <br />
              <span className='text-white'>TON SETUP</span>
            </h1>

            <p className='animate-fade-in delay-400 font-rajdhani text-white/60 text-sm leading-relaxed'>
              Anime · Jeux Vidéo · RGB LED<br />Qualité premium, livraison internationale
            </p>

            <div className='animate-fade-up delay-500 flex flex-col gap-3 w-full'>
          ...
          <div className='animate-fade-in delay-900 absolute bottom-8 left-0 right-0 flex flex-col items-center gap-2'>
            <span className='text-[10px] font-mono text-white/30 tracking-[0.3em] uppercase'>Défiler</span>
            <div className='animate-bounce-y w-px h-7 rounded-full'
      ...
      <div ref={sectionRef} className='hidden lg:block transition-colors duration-700 ease-in-out overflow-x-hidden'
        style={{ height: '100dvh' }}>
        <section className='relative flex flex-col items-center justify-start' style={{ height: '100dvh' }}>
          <div className='relative w-full flex flex-col items-center' style={{ height: '100dvh' }}>
            ...
            <div className='absolute inset-0 z-0 h-full' style={{ opacity: 1 - scrollProgress }}>
              <Image src={bgImageSrc} alt='Background' width={1920} height={1080}
                className='w-screen h-screen' style={{ objectFit: 'cover', objectPosition: 'center' }} />
            ...
            <div className='container mx-auto flex flex-col items-center justify-start relative z-10'>
              <div className='flex flex-col items-center justify-center w-full h-[100dvh] relative'>
                <div className='absolute z-0 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-none rounded-2xl'
                  ...
                  {mediaType === 'video' ? (
                    mediaSrc.includes('youtube.com') ? (
                      <div className='relative w-full h-full pointer-events-none'>
                        <iframe width='100%' height='100%'
                          ...
                          className='w-full h-full rounded-xl' frameBorder='0'
                          allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture' allowFullScreen />
                        <div className='absolute inset-0 z-10' style={{ pointerEvents: 'none' }} />
                        <div className='absolute inset-0 bg-black/30 rounded-xl'
                          style={{ opacity: 0.5 - scrollProgress * 0.3, transition: 'opacity 0.2s' }} />
                      </div>
                    ) : (
                      <div className='relative w-full h-full pointer-events-none'>
                    ...
                    <div className='relative w-full h-full'>
                      <Image src={mediaSrc} alt={title || 'Media content'} fill sizes='80vw' className='object-cover rounded-xl' />
                      <div className='absolute inset-0 bg-black/50 rounded-xl'
                        style={{ opacity: 0.7 - scrollProgress * 0.3, transition: 'opacity 0.2s' }} />
                    </div>
                  )}
                ...
                <h1 className={`flex items-center justify-center text-center gap-4 w-full relative z-10 transition-none flex-col ${textBlend ? 'mix-blend-difference' : 'mix-blend-normal'}`}>
                  <span className='text-4xl md:text-5xl lg:text-6xl font-bold text-blue-200 transition-none'
                    style={{ transform: `translateX(-${textTranslateX}vw)` }}>{firstWord}</span>
                  <span className='text-4xl md:text-5xl lg:text-6xl font-bold text-center text-blue-200 transition-none'
                    style={{ transform: `translateX(${textTranslateX}vw)` }}>{restOfTitle}</span>
                </h1>
              </div>
